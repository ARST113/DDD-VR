...
bool OpenXrSession::initializeLoaderAndInstance() {
    ...
    XrResult r = xrCreateInstance(&ci, &instance_); XR_LOGI("DDDVR/OpenXRSession", "xrCreateInstance result=%d", r); if (r != XR_SUCCESS) { lastError_ = "xrCreateInstance failed: " + std::to_string(r); return false; }
    XrSystemGetInfo sgi{XR_TYPE_SYSTEM_GET_INFO}; sgi.formFactor = XR_FORM_FACTOR_HEAD_MOUNTED_DISPLAY; r = xrGetSystem(instance_, &sgi, &systemId_); XR_LOGI("DDDVR/OpenXRSession", "xrGetSystem result=%d", r); if (r != XR_SUCCESS) { lastError_ = "xrGetSystem failed: " + std::to_string(r); return false; }
    return true;
}
...
bool OpenXrSession::prepareGraphics() {
    ...
    if (r != XR_SUCCESS) { lastError_ = "GraphicsRequirements failed: " + std::to_string(r); return false; }

...
bool OpenXrSession::hasExtension(const char* name) {
    ...
    for (auto& e : exts) {
        XR_LOGI("DDDVR/OpenXRSession", "ext=%s", e.extensionName);
...
bool OpenXrSession::initEgl() {
    ...
    EGLint num = 0;
    if (!eglChooseConfig(eglDisplay_, attrs, &eglConfig_, 1, &num) || num < 1) return false;
...
bool OpenXrSession::initialize() {
    ...
    if (!viewCfg.empty()) {
        recWidth_ = viewCfg[0].recommendedImageRectWidth;
        recHeight_ = viewCfg[0].recommendedImageRectHeight;
    }

    if (!initEgl()) {
        lastError_ = "EGL init failed";
        return false;
    }

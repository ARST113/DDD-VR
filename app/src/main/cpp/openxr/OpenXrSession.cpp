...
bool OpenXrSession::initializeLoaderAndInstance() {
    ...
    if (loaderResult != XR_SUCCESS) { XR_LOGE("DDDVR/OpenXRCheck", "LOADER_FAIL result=%d", loaderResult); lastError_ = "xrInitializeLoaderKHR failed: " + std::to_string(loaderResult); return false; }
    ...
    XrResult r = xrCreateInstance(&ci, &instance_); XR_LOGI("DDDVR/OpenXRSession", "xrCreateInstance result=%d", r); if (r != XR_SUCCESS) { XR_LOGE("DDDVR/OpenXRCheck", "INSTANCE_FAIL result=%d", r); lastError_ = "xrCreateInstance failed: " + std::to_string(r); return false; } static bool instanceOkLogged=false; if(!instanceOkLogged){ XR_LOGI("DDDVR/OpenXRCheck", "INSTANCE_OK"); instanceOkLogged=true; }
    XrSystemGetInfo sgi{XR_TYPE_SYSTEM_GET_INFO}; sgi.formFactor = XR_FORM_FACTOR_HEAD_MOUNTED_DISPLAY; r = xrGetSystem(instance_, &sgi, &systemId_); XR_LOGI("DDDVR/OpenXRSession", "xrGetSystem result=%d", r); if (r != XR_SUCCESS) { XR_LOGE("DDDVR/OpenXRCheck", "SYSTEM_FAIL result=%d", r); lastError_ = "xrGetSystem failed: " + std::to_string(r); return false; } static bool systemOkLogged=false; if(!systemOkLogged){ XR_LOGI("DDDVR/OpenXRCheck", "SYSTEM_OK"); systemOkLogged=true; }
    return true;
}
...
bool OpenXrSession::prepareGraphics() {
    ...
    if (r != XR_SUCCESS) { XR_LOGE("DDDVR/OpenXRCheck", "GL_REQUIREMENTS_FAIL result=%d", r); lastError_ = "GraphicsRequirements failed: " + std::to_string(r); return false; }
    static bool glOkLogged=false; if(!glOkLogged){ XR_LOGI("DDDVR/OpenXRCheck", "GL_REQUIREMENTS_OK"); glOkLogged=true; }
